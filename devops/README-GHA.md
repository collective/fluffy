...
2. Go to your repository at [collective/fluffy](https://github.com/collective/fluffy).
3. Click on `Settings` at the top-right corner.
4. In the left sidebar, click on `Environments`.
5. Press `New environment`.
6. Name the environment `fluffy.example.com` and hit `Configure environment`.
...
|-------------|--------------|-------------|
| DEPLOY_HOST | fluffy.example.com | The hostname or IP address of your Docker Swarm manager. |
...
- [Backend Tests Workflow](https://github.com/collective/fluffy/actions/workflows/backend.yml)
- [Frontend Tests Workflow](https://github.com/collective/fluffy/actions/workflows/frontend.yml)

Upon successful completion of the tests, Docker images for the Backend (`ghcr.io/collective/fluffy-backend`) and Frontend (`ghcr.io/collective/fluffy-frontend`) will be available.

### Initiating Manual Deployment

1. Navigate to [Manual Deployment of fluffy.example.com](https://github.com/collective/fluffy/actions/workflows/manual-deploy.yml).
...
- Initiate a new deployment using the `devops/stacks/fluffy.example.com.yml` stack.
